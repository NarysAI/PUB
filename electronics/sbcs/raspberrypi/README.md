# /pub/electronics/sbcs/raspberrypi

Raspberry Pi product catalog organized by product family.

## Families

- `boards/` — Raspberry Pi single-board computers (Pi 1–5 and Zero families).
- `accessories/` — displays and other official accessories.

The root `rpi4b`, `rpi5`, `rpi5-active-cooler`, and `rpi-touchscreen` parts are
compatibility aliases. Add new models to the appropriate family package instead
of the catalog root.

*Generated and maintained for [PartCAD](https://partcad.org/).*
