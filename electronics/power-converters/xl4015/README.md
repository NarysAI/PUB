...
The catalog uses the original STEP model. The original Fusion 360 file is retained
in `sources/` for future engineering edits. PNG renders from the source directory
were intentionally not imported.

This object is distinct from the GrabCAD `xl4015-1` variant. That variant must be
published under a separate object ID and only from its own source geometry.
